...
import SingleEffectsScreen from "../Effect/SingleEffectScreen";
...
function EffectsScreen({ navigation }) {
  ...
  const list = () => {
    return effectsData.map((effect) => {
      return (
        <Button onPress={() => navigation.navigate('Single')}>
            <CardWrapper
            direction="column"
            key={effect.id}
  ...
  return (
    <EffectsScreenWrapper>
      <ScrollView>
        <TouchableOpacity onPress={() => navigation.push('Single')}>
        ...
        <HomeStack.Navigator>
            <HomeStack.Screen name="Single" component={SingleEffectsScreen} />
        </HomeStack.Navigator>
        {/* <Button
